regVarImp <- function(model, xTrain, yTrain, xTest, fitControl, myTimeLimit, no.cores, lk_col, supress.output, mySystem, skel_outfile){
  ...
  regVarPred <- function(funcRegPred) {
    ...
    outfile<-paste("./",(lk_col-1),"in",skel_outfile,"REGControl_", paste(funcRegPred),".RData",sep="")
    
    outfileImp<-paste("./",(lk_col-1),"in",skel_outfile,"REGControl_VarImp_", paste(funcRegPred),".txt",sep="")
  ...
  if (Sys.info()[1] == "Windows"){
    ...
    resultVarImpListCombREG[model] <- mclapply(model,regVarPred, mc.preschedule=FALSE, mc.cores=no.cores, mc.cleanup=FALSE)  
