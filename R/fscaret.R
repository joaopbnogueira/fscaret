fscaret<-function(trainDF, testDF, installReqPckg=FALSE,
    ...
    cat(sprintf("\n----Starting fscaret at %s ----\n",as.character(startTime)))
    
    cat(sprintf("Size Train: %s;\tSize Test: %s;\tModels: %s\nCores: %s;\t\tSaveModel: %s;\tSkelOutfile: %s",
                  paste(as.character(dim(trainDF)),collapse=","),
                  paste(as.character(dim(testDF)),collapse=","),
                  paste(as.character(Used.funcRegPred),collapse=","),                  
                  as.character(no.cores),
                  as.character(saveModel),
                  as.character(skel_outfile)
  ...
  if(regPred==TRUE){
    ...
    regPredRES <- regVarImp(definedModels, xTrain, yTrain, xTest, fitControl, myTimeLimit, no.cores, lk_col, supress.output, mySystem, skel_outfile)
  ...
  cat(sprintf("\n---- fscaret took %s seconds to finish ----\n",as.character(as.integer(difftime(Sys.time(),startTime,units="secs")))))
